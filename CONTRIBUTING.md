...
## UI Design Gate (musu-bee)

All UI-affecting PRs must include both of the following:

1. A linked issue ID for the UI change.
2. A linked approved Pencil `.pen` artifact for the exact UI scope.

Approval contract:
- The linked issue must include an explicit CEO approval comment for the referenced `.pen`.
- The PR description must include direct links to:
  - Issue
  - Approved `.pen` artifact
  - CEO approval comment

If any of the above is missing, the PR is not merge-ready.
