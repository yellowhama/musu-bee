...
**[학습] CTO 리서치 위임이 5회 이상 연속 422(만료)로 실패할 경우:**
- CTO delegate_task 루프 즉시 중단
- CEO 메모리(memory/*.md)에 이미 저장된 리서치 결과를 근거로 목표 생성
- 메모리에 근거 있으면 charter 위반이 아님 — 전문가 지식은 이미 확보됨
...
- **전문가 리서치 없이 목표/이슈 생성 금지** (단, CEO 메모리에 근거 있으면 허용)
...
- **CTO 리서치 위임 5회 연속 422 실패 → 위임 중단, CEO 메모리 근거로 목표 직접 생성**
