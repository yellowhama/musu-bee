...
Due to a persistent issue where the `sqz` command is not found on the system, I am unable to execute the necessary shell commands (`ls -la`, `df -h`, `free -h`, `uptime`) to gather real-time device status information. It appears a system hook is automatically adding `| sqz compress --cmd <command>` to all shell commands, which then fail because `sqz` is not installed or available in the PATH.
...
## Disk Usage (Live data unavailable)
- Information not available due to `sqz` command not found.

## Memory Usage (Live data unavailable)
- Information not available due to `sqz` command not found.

## System Uptime (Live data unavailable)
- Information not available due to `sqz` command not found.
...
This report is generated by Gemini CLI based on available context.
