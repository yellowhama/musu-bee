...
from .core import ingest_core, result_has_error
from .workspace import resolve_workspace

class DirtyQueueHandler(FileSystemEventHandler):
    def __init__(self, project_root: Path, workspace):
    ...
    def _add_dirty(self, path: str):
        if not self._should_ignore(path):
            try:
                ...
                if not self.workspace.includes_path(rel_path):
                    return
                with self.lock:
...
def start_watcher(project_root: Path, debounce_seconds: int = 2, workspace=None):
    """
    데몬 모드: 파일 변경을 감지하고, Debounce 주기마다 묶어서 (Auto-Ingest) 처리합니다.
    """
    workspace = workspace or resolve_workspace(root_override=project_root)
    event_handler = DirtyQueueHandler(project_root, workspace)
    observer = Observer()
    ...
    try:
        while True:
            ...
            if dirty_files:
                ...
                result = ingest_core(project_root, dirty_files, workspace=workspace)
