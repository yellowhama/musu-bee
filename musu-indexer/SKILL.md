...
- **Command**: `musu-indexer search "<query>" --scope all|code|doc`
- **Usage**: Example: `musu-indexer search "parallel_scan" --scope code` or `musu-indexer search "workspace profile" --scope doc`. This returns ranked file paths, categories, and snippets which you should then read using standard file tools.
...
- **Command**: `musu-indexer cleanup --dry-run`
- **Usage**: Run this after changing workspace profiles, moving reference repos, or suspecting stale DB rows.

- **Command**: `musu-indexer runs --limit 5`
- **Usage**: Use this to inspect recent sync/cleanup evidence before assuming the index is wrong.

## Execution Rules
- The CLI resolves workspace in this order: `--profile`, `MUSU_INDEXER_PROFILE`, nearest `.musu-indexer.json`, `--root`, `MUSU_INDEXER_ROOT`, nearest `.git`, then `cwd`.
- In a multi-repo workspace, prefer `--profile` over raw `cwd`.
- The DB schema auto-initializes on local commands; you should not need a separate bootstrap.
