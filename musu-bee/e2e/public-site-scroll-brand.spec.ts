...
const PUBLIC_ROUTES = ["/", "/landing", "/pricing", "/install"] as const;

test.describe("public site scroll and brand", () => {
  for (const route of PUBLIC_ROUTES) {
    for (const viewport of VIEWPORTS) {
      test(`${route} scrolls and keeps the favicon mark on ${viewport.name}`, async ({ page }) => {
        await page.setViewportSize({ width: viewport.width, height: viewport.height });
        await page.goto(route, { waitUntil: "networkidle" });

        await expect(page.locator(".musu-public-scroll-root")).toBeVisible();
