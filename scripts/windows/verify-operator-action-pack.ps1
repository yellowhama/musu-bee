        ...
        try {
            ...
            try {
                ...
                if ($quickstartEntry) {
                    ...
                    Add-CheckFromCondition "second-pc transfer return zip instructions" ($quickstart -like "*.local-build\second-pc-return\*.zip*" -and $quickstart -like "*import-second-pc-return.ps1*" -and $quickstart -like "*.local-build\runtime-cleanup\*.runtime-cleanup.json*" -and $quickstart -like "*runtime_cpu_subrole_contract_ok*" -and $quickstart -like "*process_counts_by_subrole*" -and $quickstart -like "*bridge_runtime*" -and $quickstart -like "*desktop_shell*") "second-PC transfer quickstart explains return archive, cleanup evidence, and CPU subrole contract" "second-PC transfer quickstart missing return archive, cleanup, or CPU subrole instructions"
                }
                        ...
                        try {
                            ...
                            try {
                                ...
                                if ($kitReadmeEntry) {
                                    ...
                                    Add-CheckFromCondition "second-pc nested kit README wrapper" ($kitReadme -like "*run-second-pc-release-check.ps1*" -and $kitReadme -like "*.release-check.json*" -and $kitReadme -like "*.local-build\second-pc-return\*.zip*" -and $kitReadme -like "*measure-musu-idle-cpu.ps1*" -and $kitReadme -like "*.local-build\runtime-idle-cpu\*.evidence.json*" -and $kitReadme -like "*measure-musu-runtime-cpu-scenarios.ps1*" -and $kitReadme -like "*.local-build\runtime-cpu-scenarios\*" -and $kitReadme -like "*cpu_attribution*" -and $kitReadme -like "*top_processes*" -and $kitReadme -like "*process_counts_by_subrole*" -and $kitReadme -like "*runtime_cpu_subrole_contract_ok*" -and $kitReadme -like "*bridge_runtime*" -and $kitReadme -like "*desktop_shell*" -and $kitReadme -like "*webview2_helper*" -and $kitReadme -like "*show-musu-process-attribution.ps1*" -and $kitReadme -like "*machine-wide Node.js*" -and $kitReadme -like "*.local-build\runtime-cleanup\*.runtime-cleanup.json*" -and $kitReadme -like "*musu.second_pc_runtime_cleanup.v1*") "nested kit README explains release-check wrapper, runtime CPU evidence, scenario matrix subrole attribution, process attribution, and cleanup evidence" "nested kit README missing release-check wrapper/runtime CPU/scenario matrix subrole attribution/process attribution/cleanup instructions"
                                }
                                else {
