...
try {
    ...
    if (Test-Path -LiteralPath $readmePath) {
        ...
        Add-CheckFromCondition "readme second pc action" ($readme -like "*Second PC test*" -and $readme -like "*msix-install*" -and $readme -like "*second-pc-return*" -and $readme -like "*-SkipRuntimeCpuScenarioMatrix*" -and $readme -like "*-RuntimeCpuRouteTarget PRIMARY-PC*" -and $readme -like "*refuses release-grade `post-route` CPU capture without `-RuntimeCpuRouteTarget`*") "README describes second-PC action and targeted rerun boundary" "README missing second-PC action or targeted rerun boundary"
                    ...
                    if ($nestedKitEntry) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                Add-CheckFromCondition "second-pc nested kit process attribution summary" ($nestedEntries -contains "scripts\windows\show-musu-process-attribution.ps1" -and $nestedEntries -contains "scripts\windows\verify-process-attribution-summary.ps1" -and $nestedEntries -contains "scripts\windows\audit-musu-process-ownership.ps1") "nested kit includes process attribution tools" "nested kit missing process attribution tools"
                                ...
                                if ($kitReadmeEntry) {
                                    ...
                                    Add-CheckFromCondition "second-pc nested kit README wrapper" ($kitReadme -like "*run-second-pc-release-check.ps1*" -and $kitReadme -like "*-SkipRuntimeCpuScenarioMatrix*" -and $kitReadme -like "*-RuntimeCpuRouteTarget PRIMARY-PC*" -and $kitReadme -like "*refuses to run release-grade*" -and $kitReadme -like "*.release-check.json*" -and $kitReadme -like "*.local-build\second-pc-return\*.zip*" -and $kitReadme -like "*test-second-pc-route-preflight.ps1*" -and $kitReadme -like "*.local-build\second-pc-route-preflight\*.second-pc-route-preflight.json*" -and $kitReadme -like "*measure-musu-idle-cpu.ps1*" -and $kitReadme -like "*.local-build\runtime-idle-cpu\*.evidence.json*" -and $kitReadme -like "*measure-musu-runtime-cpu-scenarios.ps1*" -and $kitReadme -like "*.local-build\runtime-cpu-scenarios\*" -and $kitReadme -like "*record-route-reachability-diagnostic.ps1*" -and $kitReadme -like "*musu.route_reachability_diagnostic.v1*" -and $kitReadme -like "*.local-build\route-diagnostics\*.route-reachability-diagnostic.json*" -and $kitReadme -like "*cpu_attribution*" -and $kitReadme -like "*top_processes*" -and $kitReadme -like "*process_counts_by_subrole*" -and $kitReadme -like "*runtime_cpu_subrole_contract_ok*" -and $kitReadme -like "*bridge_runtime*" -and $kitReadme -like "*desktop_shell*" -and $kitReadme -like "*webview2_helper*" -and $kitReadme -like "*show-musu-process-attribution.ps1*" -and $kitReadme -like "*machine-wide Node.js*" -and $kitReadme -like "*.local-build\runtime-cleanup\*.runtime-cleanup.json*" -and $kitReadme -like "*musu.second_pc_runtime_cleanup.v1*") "nested kit README explains release-check wrapper, targeted rerun boundary, route preflight, runtime CPU evidence, route reachability diagnostics, scenario matrix subrole attribution, process attribution, and cleanup evidence" "nested kit README missing release-check wrapper/targeted rerun boundary/route preflight/runtime CPU/route reachability/scenario matrix subrole attribution/process attribution/cleanup instructions"
                                }
                                else {
