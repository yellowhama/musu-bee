...
Add-RegexCheck -Scope "poller" -Name "visibility wake tracks next allowed run" -Text $pollerText -Pattern 'let nextAllowedRunAt = 0[\s\S]*nextAllowedRunAt = Date\.now\(\) \+ delayMs' -Path $pollerPath -Message "Shared poller records the next allowed wake time for visibility-triggered refreshes."
Add-RegexCheck -Scope "poller" -Name "visibility wake respects scheduled backoff" -Text $pollerText -Pattern 'const remainingDelayMs = Math\.max\(0,\s*nextAllowedRunAt - Date\.now\(\)\)[\s\S]*if \(remainingDelayMs > 0\)[\s\S]*schedule\(remainingDelayMs\)' -Path $pollerPath -Message "Shared poller visibility wakes reuse the remaining scheduled backoff instead of forcing an immediate retry."
Add-NoRegexCheck -Scope "poller" -Name "visibility wake does not bypass backoff" -Text $pollerText -Pattern 'clearTimer\(\);\s*void run\(\);' -Path $pollerPath -Message "Shared poller visibility wakes do not bypass failure or hidden-tab backoff."
...
Add-RegexCheck -Scope "views-poller" -Name "views visibility wake tracks next allowed run" -Text $viewsPollerText -Pattern 'let nextAllowedRunAt = 0[\s\S]*nextAllowedRunAt = Date\.now\(\) \+ delayMs' -Path $viewsPollerPath -Message "MCP app views poller records the next allowed wake time for visibility-triggered refreshes."
Add-RegexCheck -Scope "views-poller" -Name "views visibility wake respects scheduled backoff" -Text $viewsPollerText -Pattern 'const remainingDelayMs = Math\.max\(0,\s*nextAllowedRunAt - Date\.now\(\)\)[\s\S]*if \(remainingDelayMs > 0\)[\s\S]*schedule\(remainingDelayMs\)' -Path $viewsPollerPath -Message "MCP app views poller visibility wakes reuse the remaining scheduled backoff instead of forcing an immediate retry."
Add-NoRegexCheck -Scope "views-poller" -Name "views visibility wake does not bypass backoff" -Text $viewsPollerText -Pattern 'clearTimer\(\);\s*void run\(\);' -Path $viewsPollerPath -Message "MCP app views poller visibility wakes do not bypass failure or hidden-tab backoff."
...
Add-RegexCheck -Scope "sse" -Name "bounded SSE exhausted retry guard" -Text $boundedSseText -Pattern 'let reconnectExhausted = false[\s\S]*reconnectExhausted = true' -Path $boundedSsePath -Message "Shared bounded EventSource marks reconnect exhaustion once the retry cap is hit."
Add-RegexCheck -Scope "sse" -Name "bounded SSE visibility reconnect respects exhaustion" -Text $boundedSseText -Pattern 'if \(cancelled \|\| source \|\| reconnectExhausted\) return' -Path $boundedSsePath -Message "Visibility reconnect checks do not re-arm bounded EventSource after retry exhaustion."
Add-NoRegexCheck -Scope "sse" -Name "bounded SSE visibility reconnect does not reset retry counter before reconnect" -Text $boundedSseText -Pattern 'reconnectAttempts\s*=\s*0;\s*connect\(\);' -Path $boundedSsePath -Message "Visibility reconnect checks do not silently clear the bounded reconnect cap."
