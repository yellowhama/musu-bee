...
# ── Step 5.5: Materialize musu-bee/.env.local from musu.pro (v18.B Phase 2) ──
# Without this, a fresh musu-bee build inlines empty Supabase env values and
# the middleware redirects every operator route to /login forever. Pulling
# the NEXT_PUBLIC_* values from the deployed instance lets the local UI
# authenticate against the same Supabase project musu.pro uses, with the
# same user accounts.
$beeEnvLocal = Join-Path $BeeDir ".env.local"
$publicConfigUrl = if ($env:MUSU_PUBLIC_CONFIG_URL) {
    $env:MUSU_PUBLIC_CONFIG_URL
} else {
    "https://musu.pro/api/public-config"
}

if (Test-Path $beeEnvLocal) {
    Write-Info "Step 5.5: musu-bee\.env.local already exists — skipping"
} else {
    Write-Info "Step 5.5: fetching public-config from $publicConfigUrl..."
    try {
        $config = Invoke-RestMethod -Uri $publicConfigUrl -TimeoutSec 15 -ErrorAction Stop

        $lines = @(
            "# Auto-generated by install.ps1 from $publicConfigUrl",
            "# Re-run install.ps1 to refresh (e.g. after a key rotation)."
        )
        if ($config.supabaseUrl)       { $lines += "NEXT_PUBLIC_SUPABASE_URL=$($config.supabaseUrl)" }
        if ($config.supabaseAnonKey)   { $lines += "NEXT_PUBLIC_SUPABASE_ANON_KEY=$($config.supabaseAnonKey)" }
        if ($config.paddleClientToken) { $lines += "NEXT_PUBLIC_PADDLE_CLIENT_TOKEN=$($config.paddleClientToken)" }
        if ($config.paddleEnv)         { $lines += "NEXT_PUBLIC_PADDLE_ENV=$($config.paddleEnv)" }
        # APP_URL is local even when the public config returns musu.pro —
        # OAuth callbacks must come back to this machine.
        $lines += "NEXT_PUBLIC_APP_URL=http://localhost:3001"
        # Local runtime endpoints — matches musu-bee/.env.local.example defaults.
        $lines += "MUSU_BRIDGE_URL=http://127.0.0.1:8070"
        $lines += "MUSU_WORKER_URL=http://127.0.0.1:9700"
        $lines += "NEXT_PUBLIC_MUSU_WORKER_URL=http://127.0.0.1:9700"

        # UTF-8 without BOM — Node dotenv parsers choke on the BOM marker.
        [System.IO.File]::WriteAllLines($beeEnvLocal, $lines, [System.Text.UTF8Encoding]::new($false))
        Write-Ok "musu-bee\.env.local materialized ($($lines.Count) lines)"
    } catch {
        Write-Warn "Step 5.5: public-config fetch failed — $($_.Exception.Message)"
        Write-Warn "  Without .env.local, musu-bee will not be able to authenticate."
        Write-Warn "  Manual fix: copy musu-bee\.env.local.example to musu-bee\.env.local"
        Write-Warn "             and fill NEXT_PUBLIC_SUPABASE_* from your Supabase project."
    }
}
