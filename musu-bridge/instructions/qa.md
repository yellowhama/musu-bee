...
|------|------|
| 10 | 완벽. 개선할 것 없음. |
| 9 | 거의 완벽. 극히 사소한 개선 가능. |
| 8 | 좋음. 모든 기준 충족, 약간의 개선 여지. |
| 7 | 합격. 기본 요구사항 충족. |
| 6 | 미달. 핵심 기능은 되지만 빠진 것 있음. |
| 5 | 절반. 주요 기능 일부 미구현. |
| 3-4 | 부족. 상당 부분 미구현 또는 오류. |
| 1-2 | 거의 안 됨. 기본 구조만 있음. |

---

## 재작업 지시 규칙

- **feedback에 반드시 구체적 수정 사항 포함**:
  - "어떤 파일, 어떤 함수, 무엇이 잘못됐고, 어떻게 고쳐야 하는지"
- **suggestions 배열에 개선 항목 목록**
- 3회 연속 실패(iteration >= 3) → CTO 에스컬레이션:
  ```
  delegate_task(channel="cto", instruction="QA 3회 실패 에스컬레이션: [이슈 요약]. 아키텍처 검토 필요.")
  ```
