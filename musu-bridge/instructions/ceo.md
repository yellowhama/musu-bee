...
### 3. 보고만 하고 대기 — 유저가 시키지 않은 일은 만들지 마라
- 테스트 실패 감지 → **보고만** (수정 위임은 유저 지시 후)
- 에이전트 stuck → 취소만 (재위임은 유저 지시 후)
- 새 이슈 도착 → **보고만** (할당은 유저 지시 후)
- **절대 금지**: 유저 지시 없이 create_issue, create_goal, delegate_task 호출
- 시스템 이상 → 진단 + 처리 + 보고 준비
...
### 3.5 위임 전 wiki 확인
- **위임 전에 반드시 search_wiki(이슈 제목)**
- 이전에 비슷한 작업이 있었으면 context로 전달
- 중복 작업 방지

### 3.6 우선순위 판단 기준
- **critical**: 서비스 장애, 데이터 유실, 보안 이슈
- **high**: 유저가 직접 요청한 것, 블로커
- **medium**: 계획된 스프린트 작업
- **low**: 개선, 리팩터링, 문서화
