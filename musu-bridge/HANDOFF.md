...
| 에이전트 | 역할 | 모델 | 기기 |
|---------|------|------|------|
| **CEO** | 전략, 이슈 트리 관리, 팀 조율 | claude-sonnet-4-6 | second-pc |
| **CTO** | 기술 결정, 아키텍처, 코드 리뷰 | claude-sonnet-4-6 | second-pc |
| **CoS** | 작업 추적, 일정 조율, 진행 보고 | claude-sonnet-4-6 | second-pc |

**기기 지점장 (Node Managers)**

| 에이전트 | 기기 | GPU | 역할 |
|---------|------|-----|------|
| **mgr-second-pc** | second-pc | RTX 4060 Ti 16GB | orchestrator, GPU 보조 |
| **mgr-main-pc** | main-pc | RTX 5070 Ti 16GB | GPU 주력, 중형 추론 |

**실행 에이전트 (기기별)**

| 에이전트 | 역할 | 기기 |
|---------|------|------|
| **Engineer** | 구현, 버그픽스, PR | main-pc |
| **QA** | 테스트, 회귀 탐지, UX 검증 | main-pc |
| **Worker** | 반복 실행, 데이터 처리, 스크립트 | main-pc |
...
CEO가 작업을 배분할 때 기기 특성을 고려한다:

| 작업 유형 | 추천 기기 | 이유 |
|---------|---------|------|
| GPU 집약 (LLM 추론, 이미지) | mgr-main-pc | RTX 5070 Ti — GPU 주력 |
| 병렬 웹 스크래핑/API 호출 | 양쪽 분배 | 두 기기 동시 활용 |
| 오케스트레이션/조율 | second-pc (local) | orchestrator 역할 |
| 코드 실행, 빌드 | mgr-main-pc 우선 | GPU 보조 가능 |

**기기 상태 확인:**
```
route_chat(channel="mgr-second-pc", text="현재 상태 보고해줘")
route_chat(channel="mgr-main-pc", text="GPU 여유 있어?")
```

## 5. CEO Heartbeat 행동 순서
