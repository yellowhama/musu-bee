# Windows / WSL Boundary — Operating Rules
...
1. **Source of truth = `C:\dev\musu-bee` (Windows)**.
2. **Tools see `C:\dev\musu-bee` (Windows) or `/mnt/c/dev/musu-bee` (WSL bridge to the same files)** — never `~/musu-functions/` for code changes.
...
|---|---|---|
| Code source of truth | `C:\dev\musu-bee` | Cloned from `github.com/yellowhama/musu-bee`. `main` branch only. |
| Bridge runtime data | `C:\Users\<you>\.musu\` | Created by `install.ps1`. `bridge.env`, `nodes.toml`, `musu.db`. |
...
| Indexer | `C:\dev\musu-bee\musu-indexer\.venv` | Python venv on Windows. `musu-indexer.exe` scanner binary. |
| Indexer DB | `C:\dev\musu-bee\.musu_dev.db` | SQLite + FTS5. ~50–200 MB depending on scope. |
...
## Rule 1 — Source of truth is `C:\dev\musu-bee`
...
## Rule 2 — Tools see Windows paths or `/mnt/c/...`

- **Windows tools** (PowerShell, `git`, `npm`, `npx`, `pytest`, `python`, `musu-indexer.exe`): use `C:\dev\musu-bee\…` paths.
- **WSL tools** (`bash`, `rg`, `grep`, optional Linux indexer binary): use `/mnt/c/dev/musu-bee/…`. Same file system, different driver. Editing from either side touches the same bytes.
...
Phase 2 killed two MCP servers and commented out the cron line that re-launched the auto-sync. Both are reversible (cron backup in `/tmp/crontab-backup-20260513-210559.txt`). Future tools that re-launch indexer MCP must point at `/mnt/c/dev/musu-bee` as workspace root, not `~/musu-functions`.

## How a new Claude session should orient itself

1. Read this file first.
2. `cd C:\dev\musu-bee && git log -3 --oneline` to see the most recent commits.
...
5. `musu-indexer search "<query>"` (via `C:\dev\musu-bee\musu-indexer\.venv\Scripts\musu-indexer.exe`) is the way to find code without paying for a full grep.

If you find yourself editing in `/home/hugh51/musu-functions/…`, stop. Move the change to `C:\dev\musu-bee\…` and commit there.
