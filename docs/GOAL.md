...
| 2026-05-29 v21 | Remaining manual release gates now have a single operator handoff packet generator and verifier. | Added `scripts\windows\prepare-final-operator-gate-packet.ps1` and `scripts\windows\verify-final-operator-gate-packet.ps1`; they package and verify a fresh multi-device kit, support mailbox recording template, gate docs, recorder/verifier scripts, and checksums. Latest verified packet: `.local-build\final-operator-gates\musu-final-operator-gates-1.15.0-rc.1-20260529-073523.zip` with `ok=true`, `fail_count=0`, `kit_count=1`. It does not close the gates; it reduces operator execution friction for the two remaining external checks. |
