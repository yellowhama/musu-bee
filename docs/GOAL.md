...
| 2026-05-29 v19 | Single-machine smoke is now evidence-backed in release audit. | `smoke-single-machine-beta.ps1` writes `musu.single_machine_smoke_evidence.v1`; added verifier/recorder scripts; committed evidence under `docs\evidence\single-machine\1.15.0-rc.1`. `audit-desktop-release-readiness.ps1` now reports `single_machine_verified=true`. Remaining No-Go blockers are second-PC multi-device evidence and `support@musu.pro` delivery evidence. |
...
| 2026-05-29 v33 | Go/no-go manual external gate list now includes support mailbox delivery verification. | `write-release-go-no-go.ps1` already blocked on missing `support@musu.pro` evidence, but `manual_external_gates` omitted that operator action. It now lists `support@musu.pro inbox delivery verification`, matching the blocker and final handoff operator steps. |
...
| 2026-05-29 v41 | Manual support/Store evidence now fails closed on weak operator input. | Support mailbox evidence must match the current version, include an explicit `musu-...` token, use a sender distinct from `support@musu.pro`, and pass timestamp order/future checks. Store evidence recording now requires explicit `-ProductNameReservedAt` and the verifier checks recording order/future timestamps. Final packet verification rejects stale support/Store evidence scripts without those guards. |
