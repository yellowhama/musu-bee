...
| 2026-05-29 v21 | Remaining manual release gates now have a single operator handoff packet generator. | Added `scripts\windows\prepare-final-operator-gate-packet.ps1`; it packages a fresh multi-device kit, support mailbox recording template, gate docs, recorder/verifier scripts, and checksums. Latest packet: `.local-build\final-operator-gates\musu-final-operator-gates-1.15.0-rc.1-20260529-072606.zip`. It does not close the gates; it reduces operator execution friction for the two remaining external checks. |
