...
State lock: `1.15.0-rc.1` is beta-ready for the single-machine Windows local operator path (`musu up` → dashboard doctor → Claude task smoke), but it is not public desktop release ready. A repeatable single-machine smoke script passed with evidence `20260531-212651-HUGH_SECOND` on source commit `a89f6dd`; `musu doctor` bridge health probing uses a 10s timeout and the smoke harness uses readiness retries plus `Start-Process` temp-file command capture to avoid PowerShell job/pipe hangs when `musu up` spawns a long-lived bridge. Second-PC MSIX install evidence is now recorded at `docs\evidence\msix-install\1.15.0-rc.1\20260531-165211-HUGH-MAIN.evidence.json`, but real two-machine route validation remains pending. MSIX artifact packaging is now fixed at the artifact level: `build-msix.ps1` stages `musu-desktop.exe` as the application executable while preserving `musu.exe` as the CLI alias and `musu-startup.exe` as the startup task; artifact evidence `20260531-224328-HUGH_SECOND.store-msix-desktop-artifact` passes and Store bundle `.local-build\msix\submission-bundles\store-reviewed-20260531-224352` verifies with `ok=true`. The fixed local-sideload package is now installed on `HUGH_SECOND` and passes installed desktop-entrypoint audit. Public release still remains blocked because source-fresh release packaging hit rustc OOM/pagefile pressure on this machine, single-machine smoke evidence is stale for the latest code changes, clean two-machine `desktop-open -RequireOwnedWebView2` evidence is missing, and external multi-device/support/Store evidence remains open. The release support mailbox is `musu@musu.pro`, sourced from root `SUPPORT_EMAIL`. mDNS LAN auto-discovery is opt-in via `MUSU_ENABLE_MDNS=1`, clipboard polling is opt-in via `MUSU_ENABLE_CLIPBOARD_SYNC=1`, and the logged-in cloud heartbeat defaults to `MUSU_CLOUD_HEARTBEAT_INTERVAL_SEC=300` with backoff/jitter. Frontend polling now has shared `useLowDutyPolling` coverage for major idle dashboard/panel/inbox surfaces, and writer task admission uses `Notify` instead of 50ms slot polling. Runtime CPU measurement now uses owned process-tree helper attribution instead of counting every WebView2/Node process by name and records scenario, clean git state, resource budgets, working set, private memory, and memory totals by role. One HUGH_SECOND bridge-only 60s diagnostic sample passed (`musu` max one-core CPU `0.03%`, working set `27.7MB`), but formal two-machine `desktop-open -RequireOwnedWebView2` evidence remains pending; the old desktop-open diagnostic attempt correctly failed because no MUSU-owned WebView2 process was attributed after old MSIX app activation. Process ownership is now a separate release gate: `audit-musu-process-ownership.ps1` passed locally with one MUSU runtime, zero MUSU-owned Node/WebView2 helpers, zero repo-related orphan helpers, and 13 unrelated machine-wide WebView2 processes. Startup single-instance is also a release gate: `audit-musu-startup-single-instance.ps1` passed locally with three repeated `musu up --json` calls reusing bridge PID 31208 and no repeated spawn. Operator-observed idle CPU busy-loop behavior is tracked in wiki/523; the `musu.pro` registry/rendezvous/path-selection/relay evidence contract is tracked in wiki/524, the three-blocker execution plan is tracked in wiki/525, and the Store/MSIX desktop boundary audit is tracked in wiki/526. Current qualitative completion is recorded across wiki/522, wiki/523, wiki/525, and wiki/526: local single-machine beta ~85%, Store/operator-gate infrastructure ~96%, public desktop release ~58%, full desktop GUI ~50%.
