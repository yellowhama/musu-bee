...
| — | MSIX desktop entrypoint audit | 2026-05-31 | `scripts/windows/audit-msix-desktop-entrypoint.ps1`, `evidence/msix-desktop-entrypoint/1.15.0-rc.1/20260531-214327-HUGH_SECOND.store-msix-runtime-only.evidence.json` | active |
...
2026-05-31 current status audit: `RELEASE_1_15_0_RC1_CURRENT_STATUS_AUDIT_2026_05_31.md` (wiki/522) is the current qualitative report, product spec lock, code audit, and next-step roadmap. It supersedes wiki/521 for current status while keeping wiki/521 as historical audit log. Final operator packet generation and verification now include wiki/522. Current decision remains No-Go because formal runtime idle CPU, multi-device route, support mailbox, and Store release evidence are still missing; local process ownership and startup single-instance now pass.
...
2026-05-31 MSIX desktop entrypoint audit: `MSIX_DESKTOP_ENTRYPOINT_AUDIT_2026_05_31.md` (wiki/526) records the root cause for missing owned WebView2 after MSIX activation. New script `audit-msix-desktop-entrypoint.ps1` writes `musu.msix_desktop_entrypoint_audit.v1`; current evidence `20260531-214327-HUGH_SECOND.store-msix-runtime-only` fails with `fail_count=8` because both artifact and installed package use `<Application Executable="musu.exe">`, omit `musu-desktop.exe`, and still describe `MUSU packaged CLI and bridge runtime`. `write-release-go-no-go.ps1` now reports `msix_desktop_entrypoint_verified=false`, `audit-desktop-release-readiness.ps1` reports `runtime_package_ready=false`, and `verify-store-submission-bundle.ps1` rejects the current Store bundle until the Tauri desktop shell is packaged as the MSIX app executable.
