...
2026-06-01 MUSU Pro P2P runtime update: wiki/524 now reflects that bridge runtime forwarding creates/refreshes a short-lived `musu.pro` rendezvous session before legacy direct forwarding when logged in, publishes the source endpoint, carries `rendezvous_session_id` to the target, lets the target publish candidates on receipt, closes the session after terminal success/failure, and records the session id in route evidence. `musu relay status --json` and `musu route --explain --json` now report `rendezvous_session_wired=true`. This is still not release-grade because the transport remains HTTP bearer, peer identity/QUIC-TLS proof is not wired, relay fallback is absent, and real second-PC two-sided rendezvous evidence is missing.
