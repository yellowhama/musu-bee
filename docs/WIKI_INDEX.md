...
2026-06-01 runtime route evidence and mDNS audit: `RUNTIME_ROUTE_EVIDENCE_MDNS_AUDIT_2026_06_01.md` (wiki/528) records the current implementation addendum after the operator supplied Tailscale IPv6 mDNS errors. Route evidence has been promoted from CLI-only helpers into shared `bridge::route_evidence`; `/api/tasks/delegate`, `/api/companies/{id}/run`, and workflow remote steps now write local `musu.route_evidence.v1` files under `~/.musu/route-evidence/` with timing and failure class from the real runtime attempt, then start a background best-effort submit of that evidence to `musu.pro` when an account token exists. `musu-bee/src/app/api/v1/p2p/route-evidence/route.ts` is the authenticated route-evidence receive/store/query API; it validates the contract, stores evidence in Vercel KV or an explicit local file fallback, returns `stored=true`, `evidence_id`, `release_grade`, and `blockers`, and supports filtered `GET` queries. `musu-bee/src/app/api/v1/p2p/rendezvous/*` now implements authenticated short-lived session create/read/candidate-update/approve/close endpoints for source/target candidate exchange. mDNS now default-disables both IPv6 and Tailscale interfaces unless `MUSU_MDNS_ENABLE_IPV6=1` or `MUSU_MDNS_ENABLE_TAILSCALE=1` is set. Evidence remains intentionally non-release-grade until bridge runtime route attempts use rendezvous, peer identity verification, QUIC/TLS encryption proof, account-scoped evidence ownership, and release-grade route proof are wired.
