...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260529-125448-HUGH_SECOND.evidence.json` on commit `cc336b7`; release audit reports `single_machine_verified=true`
...
- `write-release-go-no-go.ps1` includes `support@musu.pro inbox delivery verification` in `manual_external_gates`, not only as a blocker
...
- support mailbox evidence must match the current release version, use an explicit `musu-...` verification token, come from a sender distinct from `support@musu.pro`, and pass timestamp order/future checks
...
- Indexer refreshed after current single-machine evidence, `musu-system` recheck, Store run card, and second-PC handoff collector: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `899 files` and `1897 symbols`; searches for `StoreProductNameReservedAt`, `Store reservation timestamp final runner`, `inbox delivery verification`, `Support Mailbox Manual Gate Alignment`, `Remove Support Mailbox Assumption Bypass`, `public metadata skip blocker`, `final completion fail on not ready`, `git dirty release blocker`, `final packet dirty git verifier`, `final packet clean git metadata`, `multidevice evidence schema version gate`, `manual evidence fail closed`, `support verification token version gate`, `ProductNameReservedAt reservation timestamp`, `Second PC install route evidence`, `remote addr host port`, `capture checks missing`, `MUSU_RELEASE_SMOKE_OK_20260529_1255`, `PowerShell command capture hardened`, `final handoff and CI recheck`, `Store run card packet single machine refresh`, `collect second pc handoff`, `suggested_remote_addrs`, `second PC handoff JSON`, `current single machine evidence`, `musu system 1155`, `musu system recheck`, `first integration candidate`, `nurikun delivery operations remain outside MCP`, `timeout safe process`, `MUSU Microsoft Store release run card`, `HiveLink Vibe PM`, `MSIX first`, and `MICROSOFT_STORE_RELEASE_RUN_CARD` return the new scripts/docs/evidence/memory notes.
...
- Final handoff status at 2026-05-29 13:05 KST: packet verified=true, latest manifest git commit `0057a48`, `ready_for_public_desktop_release=false`; remaining blockers are clean/current second-PC MSIX install evidence, real second-PC multi-device evidence, `support@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence.
- Remote release gates are green as of 2026-05-29 13:03 KST: `Tests` run `26617175374` passed on `0057a48`; latest relevant `E2E Tests — musu-bee` and `Deploy musu-bee to Vercel` passed on `0919a83`; live `https://musu.pro/privacy` and `/support` passed public metadata verification
