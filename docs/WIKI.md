...
- Indexer refreshed after MSIX/multi-device evidence hardening: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `881 files` and `1897 symbols`; searches for `StoreProductNameReservedAt`, `Store reservation timestamp final runner`, `inbox delivery verification`, `Support Mailbox Manual Gate Alignment`, `Remove Support Mailbox Assumption Bypass`, `public metadata skip blocker`, `final completion fail on not ready`, `git dirty release blocker`, `final packet dirty git verifier`, `final packet clean git metadata`, `multidevice evidence schema version gate`, `manual evidence fail closed`, `support verification token version gate`, `ProductNameReservedAt reservation timestamp`, `Second PC install route evidence`, `remote addr host port`, and `capture checks missing` return the new scripts/docs/memory notes.
