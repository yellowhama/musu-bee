...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260531-195832-HUGH_SECOND.evidence.json` on commit `3b1b1b0`; release audit reports `single_machine_verified=true`
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260531-195832-HUGH_SECOND.evidence.json` on commit `3b1b1b0`, dashboard output `MUSU_RELEASE_SMOKE_OK`, CLI route `MUSU_CLI_ROUTE_OK`, dashboard task `d568c5f1-d15d-4cbf-8172-c4a308deaf95`, bridge `http://127.0.0.1:9818`, `doctor_overall=warn` only because the smoke binary is not the installed WindowsApps/MSIX alias.
