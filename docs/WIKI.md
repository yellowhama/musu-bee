...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260601-033633-HUGH_SECOND.evidence.json` on commit `efff90c`; release audit reports `single_machine_verified=true`
...
- `musu.pro` must evolve from registry-only discovery into registry + rendezvous + relay/tunnel control. Direct LAN/manual peers remain valid, and the bridge now ranks cached/manual/nodes candidates by path kind (`lan` -> `tailscale` -> `direct_quic`) before relay. The Store/public multi-device setup story still needs rendezvous sessions and route evidence (`lan`, `tailscale`, `direct_quic`, `relay`, or `failed`); wiki/524 locks the API/evidence contract.
...
- Final handoff contract: after the last documentation/code commit, regenerate the final operator packet from clean current HEAD and verify it. Regenerate/verify the operator action pack only after clean two-machine runtime CPU evidence passes, even though the regenerated Store submission bundle, local-sideload installed desktop-entrypoint proof, and current single-machine smoke proof now pass. Current primary clean desktop-open CPU evidence is `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260601-033734-HUGH_SECOND.desktop-open.evidence.json`; it passed with packaged `musu-desktop.exe`, six owned WebView2 helpers, owned Node count `0`, max one-core CPU `musu=0`, `webview2=0.16`, and working set `376.6MB`. Second-PC desktop CPU evidence is still missing. Final operator packet generation/verification includes wiki/522 current status, wiki/523 runtime hardening, wiki/524 P2P control-plane spec, wiki/525 runtime stabilization execution plan, and wiki/526 MSIX desktop entrypoint audit. Public desktop release remains blocked until runtime idle CPU evidence passes on two machines, real second-PC multi-device route evidence, `musu@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence are recorded.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260601-033633-HUGH_SECOND.evidence.json` on commit `efff90cc9df3419f0cadc0398e5dd63d10f235de`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260601_033608`, CLI route `MUSU_CLI_ROUTE_OK_20260601_033608`, dashboard task `759429bc-001e-4a05-9a68-88a4c44ad3c1`, bridge `http://127.0.0.1:13899`, `dashboard_task_poll_error_count=0`.
...
- 2026-06-01 mDNS/Tailscale IPv6 hardening: user-supplied `mdns_sd::service_daemon` logs showed repeated sends to `Tailscale` IPv6 link-local `ff02::fb%9:5353` failing with `os error 10065`, followed by `closed channel` errors. mDNS is still opt-in via `MUSU_ENABLE_MDNS=1`, and IPv6 mDNS is now separately opt-in via `MUSU_MDNS_ENABLE_IPV6=1`; default mDNS daemon setup disables IPv6 interfaces so `musu discover` does not hit Tailscale IPv6 by default. Local `cargo check -j 1`, `cargo build --bin musu -j 1`, and `musu discover --timeout 2` passed after this change.
...
- 2026-06-01 P2P control-plane status: `musu-rs/src/cloud/mod.rs` has rendezvous, candidate endpoint, and route-evidence DTO/client methods. `musu-rs/src/bridge/router.rs` now has shared path-kind classification and direct candidate ranking for explicit target, GPU, and OS-hint routing: `local`, `lan`, `tailscale`, then `direct_quic`, with relay still excluded until transport exists. `musu route --explain` emits `musu.route_explain.v1` without executing a task and records the selected candidate, route-kind classification, current `http_bearer` transport, `bridge_path_selection_wired=true`, `rendezvous_session_wired=false`, and release-evidence blockers. `musu relay status` emits `musu.relay_status.v1` with login/cache/client readiness, path priority, `bridge_path_selection_wired=true`, `rendezvous_session_wired=false`, `relay_transport_wired=false`, and `release_route_evidence_ready=false`. `musu.pro` local/server stub endpoints, rendezvous session creation, hardened runtime route evidence, and relay transport remain pending. Manual HTTP bearer route evidence remains intentionally rejected for release because it cannot prove identity, handshake timing, QUIC/TLS encryption, or payload transit truth.
