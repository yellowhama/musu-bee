...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260601-015424-HUGH_SECOND.evidence.json` on commit `e4509f6`; release audit reports `single_machine_verified=true`
...
- Final handoff contract: after the last documentation/code commit, regenerate the final operator packet from clean current HEAD and verify it. Regenerate/verify the operator action pack only after clean two-machine runtime CPU evidence passes, even though the regenerated Store submission bundle, local-sideload installed desktop-entrypoint proof, and current single-machine smoke proof now pass. Current primary clean desktop-open CPU evidence is `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260601-014656-HUGH_SECOND.desktop-open.evidence.json`; it passed with packaged `musu-desktop.exe`, packaged `musu.exe bridge`, six owned WebView2 helpers, owned Node count `0`, max one-core CPU `musu=0.13`, `webview2=0.16`, and working set `373.38MB`. Second-PC desktop CPU evidence is still missing. Final operator packet generation/verification includes wiki/522 current status, wiki/523 runtime hardening, wiki/524 P2P control-plane spec, wiki/525 runtime stabilization execution plan, and wiki/526 MSIX desktop entrypoint audit. Public desktop release remains blocked until runtime idle CPU evidence passes on two machines, real second-PC multi-device route evidence, `musu@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence are recorded.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260601-015424-HUGH_SECOND.evidence.json` on commit `e4509f6`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260601_015402`, CLI route `MUSU_CLI_ROUTE_OK_20260601_015402`, dashboard task `bc27c955-bd1e-41e1-8ff6-c60a38328955`, bridge `http://127.0.0.1:5076`, `dashboard_task_poll_error_count=0`.
- 2026-06-01 brand asset audit: the attractive app favicon/mark exists under `musu-bee/src-tauri/icons/` and is now tracked for web use at `musu-bee/public/images/favicon-header.png`. `musu-bee/.gitignore` previously ignored all `public/`, so clean checkouts could miss `/images/favicon-header.png` and `/agents/*.png`; required public runtime assets are now unignored/tracked. `MusuLogo` no longer points at missing `/images/logos/*` PNG lockups; it renders the app mark plus a MUSU wordmark from brand tokens.
...
- 2026-06-01 P2P control-plane status: `musu-rs/src/cloud/mod.rs` has rendezvous, candidate endpoint, and route-evidence DTO/client methods; `musu-rs/src/bridge/router.rs` is still local/manual peer selection with circuit breaker and capability hints. `musu.pro` local/server stub endpoints, `musu route --explain`, `musu relay status`, direct LAN/Tailscale path selection, and relay transport remain pending. Manual HTTP bearer route evidence remains intentionally rejected for release because it cannot prove identity, handshake timing, QUIC/TLS encryption, or payload transit truth.
- Indexer refreshed after wiki/527, the brand/public asset fix, P2P control-plane status update, current post-mDNS single-machine evidence docs, current primary runtime CPU evidence, post-runtime-CPU-gate single-machine evidence, and CoS memory note: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed 1038 files and 1917 symbols on 2026-06-01.
