...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260601-040308-HUGH_SECOND.evidence.json` on commit `e8ac5a8`; release audit reports `single_machine_verified=true`
...
- Final handoff contract: after the last documentation/code commit, regenerate the final operator packet from clean current HEAD and verify it. Regenerate/verify the operator action pack only after clean two-machine runtime CPU evidence passes, even though the regenerated Store submission bundle, local-sideload installed desktop-entrypoint proof, and current single-machine smoke proof now pass. Current primary clean desktop-open CPU evidence is `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260601-040413-HUGH_SECOND.desktop-open.evidence.json`; it passed with packaged `musu-desktop.exe`, six owned WebView2 helpers, owned Node count `0`, max one-core CPU `musu=0`, `webview2=0.21`, and working set `369.66MB`. Second-PC desktop CPU evidence is still missing. Final operator packet generation/verification includes wiki/522 current status, wiki/523 runtime hardening, wiki/524 P2P control-plane spec, wiki/525 runtime stabilization execution plan, and wiki/526 MSIX desktop entrypoint audit. Public desktop release remains blocked until runtime idle CPU evidence passes on two machines, real second-PC multi-device route evidence, `musu@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence are recorded.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260601-040308-HUGH_SECOND.evidence.json` on commit `e8ac5a88c1dd9437c965f2d2e2f2c3b331596c2a`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260601_040245`, CLI route `MUSU_CLI_ROUTE_OK_20260601_040245`, dashboard task `612c8aff-616e-4227-89dc-7023a77d4830`, bridge `http://127.0.0.1:13800`, `dashboard_task_poll_error_count=0`.
...
- 2026-06-01 mDNS/Tailscale hardening: user-supplied `mdns_sd::service_daemon` logs showed repeated sends to `Tailscale` IPv6 link-local `ff02::fb%9:5353` failing with `os error 10065`, followed by `closed channel` errors. mDNS is still opt-in via `MUSU_ENABLE_MDNS=1`; IPv6 mDNS is separately opt-in via `MUSU_MDNS_ENABLE_IPV6=1`; Tailscale mDNS interfaces are separately opt-in via `MUSU_MDNS_ENABLE_TAILSCALE=1`. Current source should not hit Tailscale IPv6 mDNS in default bridge startup.
...
- 2026-06-01 P2P control-plane status: `musu-rs/src/cloud/mod.rs` has rendezvous, candidate endpoint, and route-evidence DTO/client methods. `musu-rs/src/bridge/router.rs` now has shared path-kind classification and direct candidate ranking for explicit target, GPU, and OS-hint routing: `local`, `lan`, `tailscale`, then `direct_quic`, with relay still excluded until transport exists. `musu route --explain` emits `musu.route_explain.v1` without executing a task and records the selected candidate, route-kind classification, current `http_bearer` transport, `bridge_path_selection_wired=true`, `rendezvous_session_wired=false`, and release-evidence blockers. `musu-rs/src/bridge/route_evidence.rs` is now the shared CLI/runtime route evidence writer. `musu route --route-evidence-path <path>` writes `musu.route_evidence.v1` from the actual CLI attempt, and bridge remote forwarding from `/api/tasks/delegate`, `/api/companies/{id}/run`, and workflow remote steps writes local `~/.musu/route-evidence/<task_id>.route-evidence.json` from the actual runtime attempt. Runtime forwarding now starts a background best-effort submit of the same evidence to `musu.pro` when a `~/.musu/token` account token exists. `musu-bee/src/app/api/v1/p2p/route-evidence/route.ts` is the authenticated route-evidence receive/store/query API; it validates `musu.route_evidence.v1`, stores the record in Vercel KV or an explicit local file fallback, returns `stored=true`, `evidence_id`, `release_grade`, and `blockers`, and supports filtered `GET` queries. `musu relay status` emits `musu.relay_status.v1` with login/cache/client readiness, path priority, `bridge_path_selection_wired=true`, `rendezvous_session_wired=false`, `relay_transport_wired=false`, and `release_route_evidence_ready=false`. Rendezvous session creation, account-scoped evidence ownership/export, release-grade identity/encryption proof, and relay transport remain pending. Manual HTTP bearer route evidence remains intentionally rejected for release because it cannot prove peer identity, QUIC/TLS encryption, or payload transit truth.
