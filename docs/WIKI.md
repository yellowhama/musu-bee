...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260530-085123-HUGH_SECOND.evidence.json` on commit `213b69e`; release audit reports `single_machine_verified=true`
...
- second-PC return card script: `scripts\windows\show-second-pc-return-card.ps1` reads returned `.local-build\second-pc-handoff\*.handoff.json`, chooses a `suggested_remote_addrs` candidate, and prints the exact primary-side MSIX record / multi-device smoke / multi-device record commands
...
- `verify-final-operator-gate-packet.ps1` also inspects each bundled second-PC kit for the local-sideload MSIX, public signing cert, install/verify script, MSIX install evidence capture/verify/record scripts, second-PC handoff collector, multi-device smoke script, multi-device evidence verify/record scripts, and README instructions for both install evidence and multi-device evidence capture.
...
- Indexer refreshed after support mailbox correction, current single-machine evidence, operator handoff card, final qualitative audit, post-audit single-machine refresh, second-PC kit verifier hardening, doctor-timeout smoke refresh, second-PC return card, smoke harness process-capture hardening, operator action pack scripts, post-action-pack single-machine evidence refresh, handoff status action-pack verification, the second-PC release-check wrapper, beta checklist current-evidence pointer correction, `musu-system` 2026-05-30 recheck, and Store submission bundle verifier: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `951 files` and `1897 symbols`; searches for `wiki 521`, `final qualitative audit next steps`, `musu pro final operator packet`, `operator action pack`, `prepare operator action pack`, `verify operator action pack`, `verify store submission bundle`, `run second pc release check`, `second PC release-check wrapper`, `beta checklist current evidence pointer`, `musu system 2026 05 30 recheck`, `action_pack.verified`, `MSIX install evidence`, `public desktop release evidence gates`, `show operator handoff card`, `show second pc return card`, `support email correction`, `SUPPORT_EMAIL`, `release config support mailbox`, `MUSU_RELEASE_SMOKE_OK_20260530_085059`, `MUSU_CLI_ROUTE_OK_20260530_085059`, `PowerShell job pipe hang`, `Start-Process temp-file command capture`, `bridge doctor false-negative`, `doctor timeout`, `second PC kit verifier hardening`, `StoreProductNameReservedAt`, `Second PC install route evidence`, `remote addr host port`, `capture checks missing`, `collect second pc handoff`, `suggested_remote_addrs`, `musu system recheck`, and `MICROSOFT_STORE_RELEASE_RUN_CARD` return the new scripts/docs/evidence/memory notes.
...
- Final handoff contract: after the last documentation/code commit, regenerate the final operator packet from clean current HEAD, verify it, regenerate/verify the operator action pack, then run `show-final-release-handoff-status.ps1 -Json`; public desktop release remains blocked until clean/current second-PC MSIX install evidence, real second-PC multi-device evidence, `musu@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence are recorded.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260530-085123-HUGH_SECOND.evidence.json` on commit `213b69e`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260530_085059`, CLI route `MUSU_CLI_ROUTE_OK_20260530_085059`, dashboard task `14c9e9be-73b7-494f-9194-fd87d8f4b252`, bridge `http://127.0.0.1:12427`, `doctor_overall=warn` only because the smoke binary is not the installed WindowsApps/MSIX alias.
