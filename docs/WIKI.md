...
- 2026-06-01 P2P control-plane status: `musu-rs/src/cloud/mod.rs` has rendezvous, candidate endpoint, and route-evidence DTO/client methods; `musu-rs/src/bridge/router.rs` is still local/manual peer selection with circuit breaker and capability hints. `musu route --explain` now emits `musu.route_explain.v1` without executing a task and records the selected candidate, route-kind classification, current `http_bearer` transport, and release-evidence blockers. `musu relay status` now emits `musu.relay_status.v1` with login/cache/client readiness and explicitly reports `bridge_path_selection_wired=false`, `relay_transport_wired=false`, and `release_route_evidence_ready=false`. `musu.pro` local/server stub endpoints, direct LAN/Tailscale path selection, and relay transport remain pending. Manual HTTP bearer route evidence remains intentionally rejected for release because it cannot prove identity, handshake timing, QUIC/TLS encryption, or payload transit truth.
- Indexer refreshed after wiki/527, the brand/public asset fix, generated static logo lockups, P2P control-plane status update, current post-mDNS single-machine evidence docs, current primary runtime CPU evidence, post-runtime-CPU-gate single-machine evidence, second-PC return CPU evidence flow, post-logo single-machine evidence, refreshed primary runtime CPU evidence, route/relay diagnostic CLI surfaces, post-route-diagnostics smoke/CPU evidence, and CoS memory note: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed 1047 files and 1930 symbols on 2026-06-01.
