...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260601-084028-HUGH_SECOND.evidence.json` on commit `a1ee33fa0c8e3b68e85dc4b48077134ec5dd99ac`; release go/no-go reports `single_machine_verified=true`
...
- Final handoff contract: after the last documentation/code commit, regenerate the final operator packet from clean current HEAD and verify it. Regenerate/verify the operator action pack only after clean two-machine runtime CPU evidence passes, even though the regenerated Store submission bundle, local-sideload installed desktop-entrypoint proof, and current single-machine smoke proof now pass. Current single-machine smoke proof is `docs\evidence\single-machine\1.15.0-rc.1\20260601-084028-HUGH_SECOND.evidence.json` on commit `a1ee33fa0c8e3b68e85dc4b48077134ec5dd99ac`. Current primary clean desktop-open CPU evidence is `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260601-082822-HUGH_SECOND.desktop-open.evidence.json`; it passed with packaged `musu-desktop.exe`, six owned WebView2 helpers, owned Node count `0`, max one-core CPU `musu=0`, `webview2=0.16`, and working set `339.85MB`. Second-PC desktop CPU evidence is still missing. Final operator packet generation/verification includes wiki/522 current status, wiki/523 runtime hardening, wiki/524 P2P control-plane spec, wiki/525 runtime stabilization execution plan, and wiki/526 MSIX desktop entrypoint audit. Public desktop release remains blocked until runtime idle CPU evidence passes on two machines, real second-PC multi-device route evidence, `musu@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence are recorded.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260601-084028-HUGH_SECOND.evidence.json` on commit `a1ee33fa0c8e3b68e85dc4b48077134ec5dd99ac`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260601_084005`, CLI route `MUSU_CLI_ROUTE_OK_20260601_084005`, dashboard task `5ac5baa6-471f-4633-9a57-9e3a87a20c7a`, bridge `http://127.0.0.1:13167`, and `cli_route_checked=true`.
...
- 2026-06-01 operator re-supplied mDNS/Tailscale log: the repeated `2026-05-31T16:09:08Z` through `16:10:24Z` `ff02::fb%9` failures match the already-classified Windows/Tailscale IPv6 mDNS issue. If the installed desktop still emits these logs with current source defaults, treat it as stale installed bits or an explicit `MUSU_ENABLE_MDNS=1` plus IPv6/Tailscale opt-in environment problem; the Store-candidate default path must keep mDNS, IPv6 mDNS, and Tailscale mDNS disabled.
