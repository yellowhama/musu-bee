...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260530-100818-HUGH_SECOND.evidence.json` on commit `7294d65`; release audit reports `single_machine_verified=true`
...
- `verify-final-operator-gate-packet.ps1` also inspects each bundled second-PC kit for the local-sideload MSIX, public signing cert, install/verify script, MSIX install evidence capture/verify/record scripts, second-PC handoff collector, multi-device smoke script, multi-device evidence verify/record scripts, return-archive instructions, and README instructions for both install evidence and multi-device evidence capture.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260530-100818-HUGH_SECOND.evidence.json` on commit `7294d65`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260530_1008`, CLI route `MUSU_CLI_ROUTE_OK_20260530_1008`, dashboard task `695f1e1d-1d1b-46bd-8783-3eebb216842a`, bridge `http://127.0.0.1:2217`, `doctor_overall=warn` only because the smoke binary is not the installed WindowsApps/MSIX alias.
