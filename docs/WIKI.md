...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260531-192015-HUGH_SECOND.evidence.json` on commit `5211ff2`; release audit reports `single_machine_verified=true`
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260531-192015-HUGH_SECOND.evidence.json` on commit `5211ff2`, dashboard output `MUSU_RELEASE_SMOKE_OK`, CLI route `MUSU_CLI_ROUTE_OK`, dashboard task `986733bf-c10c-4434-bd8e-d9b0512ce193`, bridge `http://127.0.0.1:9818`, `doctor_overall=warn` only because the smoke binary is not the installed WindowsApps/MSIX alias.
