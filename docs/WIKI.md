...
- Indexer refreshed after removing the support mailbox assumption bypass: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `873 files` and `1897 symbols`; searches for `StoreProductNameReservedAt`, `Store reservation timestamp final runner`, `inbox delivery verification`, `Support Mailbox Manual Gate Alignment`, and `Remove Support Mailbox Assumption Bypass` return the new scripts/docs/memory notes.
