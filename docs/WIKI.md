...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260531-205509-HUGH_SECOND.evidence.json` on commit `df49459`; release audit reports `single_machine_verified=true`
...
- Runtime idle CPU is now a release blocker in `write-release-go-no-go.ps1`. Use `scripts\windows\measure-musu-idle-cpu.ps1 -SampleSeconds 60 -MaxOneCorePercent 5 -IncludeNode -IncludeWebView2 -FailOnHot -Json` on primary and second PC with MUSU open and idle; public beta requires two machine samples, at least one MUSU runtime process in each run, and <=5% of one logical CPU across MUSU/Node.js/WebView2 while idle. The sampler defaults to MUSU-owned descendants plus repo-related helpers, records `helper_process_scope`, uses native Windows parent-process lookup instead of WMI/CIM, and leaves `-IncludeUnrelatedHelpers` for whole-machine diagnostics.
...
- Final handoff contract: after the last documentation/code commit, regenerate the final operator packet from clean current HEAD, verify it, regenerate/verify the operator action pack, then run `show-final-release-handoff-status.ps1 -Json`; final operator packet generation/verification includes wiki/522 current status, wiki/523 runtime hardening, wiki/524 P2P control-plane spec, and wiki/525 runtime stabilization execution plan. Public desktop release remains blocked until runtime idle CPU evidence, real second-PC multi-device route evidence, `musu@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence are recorded.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260531-205509-HUGH_SECOND.evidence.json` on commit `df49459`, dashboard output `MUSU_RELEASE_SMOKE_OK`, CLI route `MUSU_CLI_ROUTE_OK`, dashboard task `5b75f3cc-b140-43e9-8882-9e8822075128`, bridge `http://127.0.0.1:9818`, `doctor_overall=warn` only because the smoke binary is not the installed WindowsApps/MSIX alias.
