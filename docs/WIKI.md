...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260529-185958-HUGH_SECOND.evidence.json` on commit `242d75f`; release audit reports `single_machine_verified=true`
...
- Indexer refreshed after support mailbox correction, current single-machine evidence, operator handoff card, final qualitative audit, post-audit single-machine refresh, second-PC kit verifier hardening, doctor-timeout smoke refresh, second-PC return card, smoke harness process-capture hardening, and operator action pack scripts: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `934 files` and `1897 symbols`; searches for `wiki 521`, `final qualitative audit next steps`, `musu pro final operator packet`, `operator action pack`, `prepare operator action pack`, `verify operator action pack`, `MSIX install evidence`, `public desktop release evidence gates`, `show operator handoff card`, `show second pc return card`, `support email correction`, `SUPPORT_EMAIL`, `release config support mailbox`, `MUSU_RELEASE_SMOKE_OK_20260529_185935`, `MUSU_CLI_ROUTE_OK_20260529_185935`, `PowerShell job pipe hang`, `Start-Process temp-file command capture`, `bridge doctor false-negative`, `doctor timeout`, `second PC kit verifier hardening`, `StoreProductNameReservedAt`, `Second PC install route evidence`, `remote addr host port`, `capture checks missing`, `collect second pc handoff`, `suggested_remote_addrs`, `musu system recheck`, and `MICROSOFT_STORE_RELEASE_RUN_CARD` return the new scripts/docs/evidence/memory notes.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260529-185958-HUGH_SECOND.evidence.json` on commit `242d75f`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260529_185935`, CLI route `MUSU_CLI_ROUTE_OK_20260529_185935`, dashboard task `3cab5be8-1abf-40c0-91ad-3f5d2da33bcb`, bridge `http://127.0.0.1:9218`, `doctor_overall=warn` only because the smoke binary is not the installed WindowsApps/MSIX alias.
