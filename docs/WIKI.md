...
- Indexer refreshed after making skipped public metadata a blocker: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `874 files` and `1897 symbols`; searches for `StoreProductNameReservedAt`, `Store reservation timestamp final runner`, `inbox delivery verification`, `Support Mailbox Manual Gate Alignment`, `Remove Support Mailbox Assumption Bypass`, and `public metadata skip blocker` return the new scripts/docs/memory notes.
