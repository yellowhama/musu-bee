...
- Runtime idle CPU is now a release blocker in `write-release-go-no-go.ps1`. Use `scripts\windows\measure-musu-idle-cpu.ps1 -SampleSeconds 60 -MaxOneCorePercent 5 -IncludeNode -IncludeWebView2 -FailOnHot -Json` on primary and second PC with MUSU open and idle; public beta requires two machine samples, at least one MUSU runtime process in each run, and <=5% of one logical CPU across MUSU/Node.js/WebView2 while idle.
