...
- current-version Store-reviewed package: regenerated for `1.15.0-rc.1` as `musu_1.15.0.0_x64_store-reviewed-immediate-registration.msix`, but now blocked from public submission because the MSIX launches `musu.exe`, omits `musu-desktop.exe`, and is runtime-only
...
- MSIX desktop entrypoint is now a release blocker in `write-release-go-no-go.ps1`. Use `scripts\windows\audit-msix-desktop-entrypoint.ps1 -StartupContract store-reviewed-immediate-registration -ExpectedApplicationExecutable musu-desktop.exe -RequireInstalledPackage -Json`; current evidence `docs\evidence\msix-desktop-entrypoint\1.15.0-rc.1\20260531-214327-HUGH_SECOND.store-msix-runtime-only.evidence.json` fails because both artifact and installed package launch `musu.exe`, do not contain `musu-desktop.exe`, and still describe a CLI/bridge runtime package. The Tauri shell exists separately at `musu-bee\src-tauri\target\release\musu-desktop.exe`, but it is not inside the current MSIX.
...
- Frontend idle polling hardening now includes DashboardClient, NodePanel, and agents-surface recursive timers at 30s visible / 120s hidden, preventing overlapping fixed-interval refresh loops on long-running desktop sessions.
...
- Store submission bundle verification exists: `scripts\windows\verify-store-submission-bundle.ps1`; `prepare-store-submission-bundle.ps1` now writes `SHA256SUMS.txt`, `audit-desktop-release-readiness.ps1` verifies the latest bundle, and `prepare-operator-action-pack.ps1` refuses to build from an invalid Store submission bundle. As of wiki/526, the current Store bundle is invalid because the MSIX desktop entrypoint audit fails.
...
- Final handoff contract: after the last documentation/code commit, regenerate the final operator packet from clean current HEAD and verify it. Regenerate/verify the operator action pack only after the Store submission bundle passes the MSIX desktop-entrypoint audit; the current runtime-only bundle intentionally blocks action-pack generation. Final operator packet generation/verification includes wiki/522 current status, wiki/523 runtime hardening, wiki/524 P2P control-plane spec, wiki/525 runtime stabilization execution plan, and wiki/526 MSIX desktop entrypoint audit. Public desktop release remains blocked until `msix_desktop_entrypoint_verified`, runtime idle CPU evidence, real second-PC multi-device route evidence, `musu@musu.pro` inbox delivery evidence, and Partner Center/Microsoft Store release evidence are recorded.
