...
- Store release evidence scripts exist: `scripts\windows\verify-store-release-evidence.ps1` and `scripts\windows\record-store-release-verification.ps1`; Store release evidence now records explicit Partner Center product name reservation state, not only submission/certification fields
...
- Indexer refreshed after Store product-name evidence updates: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `870 files` and `1897 symbols`; searches for `msix install evidence`, `capture-msix-install-evidence`, `MsixInstallEvidencePath`, `0919 musu system recheck`, `product_name_reserved`, and `Store product name evidence` return the new scripts/docs/memory notes.
