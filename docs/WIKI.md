...
- current machine-readable single-machine evidence exists at `docs\evidence\single-machine\1.15.0-rc.1\20260530-060739-HUGH_SECOND.evidence.json` on commit `d7e745f`; release audit reports `single_machine_verified=true`
...
- Indexer refreshed after support mailbox correction, current single-machine evidence, operator handoff card, final qualitative audit, post-audit single-machine refresh, second-PC kit verifier hardening, doctor-timeout smoke refresh, second-PC return card, smoke harness process-capture hardening, operator action pack scripts, post-action-pack single-machine evidence refresh, and handoff status action-pack verification: `musu indexer sync --work-dir F:\workspace\musu-bee --name musu-bee` indexed `943 files` and `1897 symbols`; searches for `wiki 521`, `final qualitative audit next steps`, `musu pro final operator packet`, `operator action pack`, `prepare operator action pack`, `verify operator action pack`, `action_pack.verified`, `MSIX install evidence`, `public desktop release evidence gates`, `show operator handoff card`, `show second pc return card`, `support email correction`, `SUPPORT_EMAIL`, `release config support mailbox`, `MUSU_RELEASE_SMOKE_OK_20260530_060700`, `MUSU_CLI_ROUTE_OK_20260530_060700`, `PowerShell job pipe hang`, `Start-Process temp-file command capture`, `bridge doctor false-negative`, `doctor timeout`, `second PC kit verifier hardening`, `StoreProductNameReservedAt`, `Second PC install route evidence`, `remote addr host port`, `capture checks missing`, `collect second pc handoff`, `suggested_remote_addrs`, `musu system recheck`, and `MICROSOFT_STORE_RELEASE_RUN_CARD` return the new scripts/docs/evidence/memory notes.
...
- Current single-machine evidence: `docs\evidence\single-machine\1.15.0-rc.1\20260530-060739-HUGH_SECOND.evidence.json` on commit `d7e745f`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260530_060700`, CLI route `MUSU_CLI_ROUTE_OK_20260530_060700`, dashboard task `146d0165-5f4a-428e-9d74-f3ea0b05dbad`, bridge `http://127.0.0.1:12305`, `doctor_overall=warn` only because the smoke binary is not the installed WindowsApps/MSIX alias.
