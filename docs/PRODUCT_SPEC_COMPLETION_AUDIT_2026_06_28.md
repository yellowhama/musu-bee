...
The prior 10-blocker package-bound evidence refresh remains useful historical
evidence for commit `0db4b39e`, but it is no longer current for HEAD. The new
...
2026-06-30 21:24 KST remote file CLI physical proof policy blocker: the next
real sibling `musu ls/get/put` proof was attempted from `HUGH_SECOND` to
`hugh-main` and produced evidence at
`docs/evidence/remote-file-cli/1.15.0-rc.22/20260630-212409-HUGH_SECOND-to-hugh-main.remote-file-cli-proof.json`.
The proof is `ok=false`, but the failure mode is different and more precise:
the target bridge returned `forbidden` file policy responses, not
`unauthorized: invalid bearer`. `musu put` was rejected because file writes are
...
| INFO | HUGH_SECOND package-bound freshness is restored after the remote file CLI source fix. | Post-fix package evidence refresh records MSIX install, single-machine smoke, process ownership, startup/desktop single-instance, desktop-open idle CPU, and full runtime CPU matrix on `HUGH_SECOND`; latest clean gate is back to 10 blockers instead of the 15-blocker source-freshness reset. | Local package freshness is no longer the current blocker. | Keep package-bound evidence fresh after any future source or package change. |
