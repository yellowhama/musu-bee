...
| `scripts\windows\configure-musu-pro-p2p-env.ps1` | n/a | Operator helper | Sets `KV_REST_API_URL` and `KV_REST_API_TOKEN` for the GitHub/Vercel deploy path without printing secret values, optionally triggers `deploy-musu-bee.yml`, and then lets `show-musu-pro-p2p-env-status.ps1` confirm whether the hosted blocker moved past `p2p_relay_lease_kv_not_configured`. |
