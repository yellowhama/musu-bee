...
| V26-W7 | `musu peer register` worker helper | wiki/510 | `musu-rs/src/peer/` | `musu peer register --type {ollama\|comfyui\|script} --start "<cmd>"` + systemd/launchd/SCM cross-platform + local node manifest | LOW | 300 → **1,140** | V24 R6 installer service registration |
