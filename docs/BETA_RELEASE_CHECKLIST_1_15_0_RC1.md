...
Current-head CPU matrix after the operator-requested SaaS deep dive:
...
This matrix was captured from clean commit
`924a2b1f680f6c9041bc01c2307d011c626276fd` with `git_dirty=false` and
`musu_exe_release_identity=true`. All five scenarios passed with hot process
count `0`, MUSU CPU `0`, Node CPU `0`, owned WebView2 max `0.08`, owned
process count `8`, owned WebView2 helper count `6`, and max working set
`370.46MB`. The local post-route probe succeeded with token
...
`write-release-go-no-go.ps1` verifies this diagnostic path with
`-RejectSelfPostRouteTarget`, so a matrix whose `route_probe.target` equals
`operator_machine` cannot count as second-PC route-attempt CPU evidence. The
same verifier also requires `route_probe.command` and `route_probe.arguments`
to bind the same target through `--target <target>` or `--target=<target>`.
For every `post-route` matrix, the verifier also requires non-empty
`expected_token`, command/arguments bound to `--wait` with that token, and
successful route output containing the token. An explicitly allowed failed
route diagnostic must record a numeric non-zero exit code.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\windows\verify-runtime-cpu-scenario-matrix.ps1 -EvidencePath <MATRIX_JSON> -RequirePostRouteProbe -ExpectedPostRouteTarget PRIMARY-PC -RejectSelfPostRouteTarget -AllowFailedPostRouteProbe -Json
```
...
The targeted diagnostic path must also reject self-target probes;
`route_probe.target` must not match `operator_machine`, and the recorded
command/arguments must bind the same target with `--target`. Every post-route
probe must bind `--wait` to the expected token, and successful probe output must
contain that token. If the targeted diagnostic uses the explicitly allowed
failed-probe path, the route probe must record a numeric non-zero exit code.
...
Qualitative audit found no high/medium code issue. No code changed for this
evidence refresh. The local desktop-open runtime remains healthy on one
machine, but the release remains No-Go because second-PC route/CPU/matrix,
hosted MUSU.PRO P2P/relay proof, public metadata recheck, support mailbox
proof, and Store/Partner Center proof are still required.
...
- rejects raw payload byte fields in rendezvous creation with
  `rendezvous_payload_bytes_not_accepted`
- rejects raw payload byte fields in candidate exchange with
  `rendezvous_candidates_payload_bytes_not_accepted`
- unknown fields fail with invalid request errors and concrete field paths
...
Qualitative audit found no high/medium issue. This closes a P2P bootstrap
input-boundary gap only. Release remains No-Go on second-PC route/CPU/matrix,
hosted MUSU.PRO P2P/relay proof, public metadata recheck, support mailbox
proof, and Store/Partner Center proof.
...
include structured release status fields instead of terse error-only JSON.

Key fields:

- relay connect failure paths: `schema=musu.relay_connect.v1`,
  `relay_connect_accepted=false`, `payload_transported=false`,
  `lease_verified=false`
- release payload failure paths: `schema=musu.relay_payload_preflight.v1`,
...
release-evidence hardening only; the release relay tunnel payload transport
remains unimplemented and the preview queue remains non-release-grade.
...
Hosted P2P remains blocked:

- verifier: `ok=false`, `fail_count=42`
- logged-in runtime evidence: `false`
- owner scope: `false`
- release-grade relay lease store: `false`
- relay route evidence count: `0`
...
Qualitative audit found no high/medium issue. This does not change the product
split: MUSU Desktop is the local executor; MUSU.PRO is remote input,
...
Clean current HEAD `dd0e409ee3a8ade2153bb858f74c4c5a0abf5bc2` was sampled on
...
Qualitative audit: no high/medium issue found. The reported 20% idle CPU loop
is not reproduced on this current packaged one-machine desktop-open state.
Public release remains No-Go until second-PC CPU/matrix, real route evidence,
hosted P2P control-plane/relay proof, support mailbox, and Store evidence are
complete.
...
Current HEAD was rebuilt, reinstalled, and rechecked after the Rust release
relay tunnel submit metadata gate changed runtime source.

Package and install:

- MSIX workflow: release x64, `local-sideload-manual`
- install mode: current-user replace existing
...
- known local warning: this shell still resolves
  `C:\Users\empty\.cargo\bin\musu.exe` before the WindowsApps alias; evidence
  commands used explicit `C:\Users\empty\AppData\Local\Microsoft\WindowsApps\musu.exe`
...
- HUGH-MAIN target-route CPU attempt:
...
- single-machine: `ok=true`, surface `local-bridge-only`, bridge
  `http://127.0.0.1:14361`, CLI route checked
- process ownership: packaged runtime `1`, owned Node `0`, owned WebView2 `0`,
  repo orphan helpers `0`
- startup single-instance: repeated `musu up` reused bridge PID `34860`
- desktop single-instance: one packaged desktop shell after three activations
- desktop-open idle CPU: `ok=true`, `60.03s`, hot `0`, owned WebView2 `6`,
  WebView2 max one-core `0.18`, total working set `361.93MB`
- five-state matrix: `ok=true`, route probe success, all required scenarios
  passed, max WebView2 one-core `0.16`
- target-route CPU: `ok=true`, target `HUGH-MAIN`, failed route output
  allowed, post-route CPU passed, WebView2 max one-core `0.03`
...
- second machine idle CPU evidence still required (`1/2`)
- second machine five-state CPU matrix still required (`1/2`)
- real second-PC multi-device evidence missing
- hosted MUSU.PRO P2P/relay proof missing
- support mailbox proof missing
- Store/Partner Center proof missing

Canonical report:

- `docs\RELEASE_1_15_0_RC1_CURRENT_HEAD_LOCAL_DESKTOP_EVIDENCE_AFTER_RELAY_METADATA_GATE_2026_06_07.md`
  (wiki/945)
