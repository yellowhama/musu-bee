...
| Public desktop release readiness | ~66% | MSIX desktop entrypoint, primary CPU proof, runtime relay lease request wiring, and persisted relay fallback evidence are stronger, but clean two-machine desktop/WebView2 idle CPU, multi-device route, support mailbox, Store approval, QUIC/TLS proof, and relay/tunnel transport remain open. |
