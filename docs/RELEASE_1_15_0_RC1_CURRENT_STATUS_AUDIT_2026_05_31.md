...
| Latest go/no-go after source commit | expected state after this documentation/evidence pass remains `ready=false`, `single_machine=true`, runtime idle CPU `1/2`, runtime CPU scenario matrix `1/2`, multi-device false, support false, Store false, and `manifest_dirty=false`; rerun after final commit |
