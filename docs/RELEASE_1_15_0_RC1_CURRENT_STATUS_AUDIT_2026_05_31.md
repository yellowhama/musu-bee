...
| Public desktop release readiness | ~68% | MSIX desktop entrypoint, primary CPU proof, primary 4-state CPU matrix, runtime relay lease request wiring, and persisted relay fallback evidence are stronger, but clean two-machine desktop/WebView2 idle CPU, second-PC matrix, multi-device route, support mailbox, Store approval, QUIC/TLS proof, and relay/tunnel transport remain open. |
