...
| Public desktop release readiness | ~67% | MSIX desktop entrypoint, primary CPU proof, primary 4-state CPU matrix, public site deployment, runtime relay lease request wiring, and persisted relay fallback evidence are stronger, but clean two-machine desktop/WebView2 idle CPU, second-PC matrix, multi-device route, support mailbox, Store approval, QUIC/TLS proof, relay/tunnel transport, and production P2P control auth remain open. |
...
| Relay lease production audit | `musu relay status --json` shows logged-in control-plane wiring, but `musu relay leases --json` against `https://musu.pro` currently fails with `p2p_control_auth_not_configured`; production P2P control auth must be fixed before relay lease evidence can be trusted |
...
12. **musu.pro network role**: the product now has hosted registry/rendezvous/route-evidence/relay-lease policy wiring, a runtime fail-closed relay lease request after direct failure, failed route evidence that records the relay fallback evaluation, and a CLI audit surface for relay lease records. Direct LAN/manual peers remain valid, but cannot be the only public multi-device setup story. Relay payload transport remains unwired and must not become the default data path. Production P2P control auth is still blocked by `p2p_control_auth_not_configured` until the account/device token path is validated or a scoped control token is configured.
