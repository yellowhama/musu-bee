...
| Public desktop release readiness | ~65% | MSIX desktop entrypoint, primary CPU proof, and runtime relay lease request wiring are stronger, but clean two-machine desktop/WebView2 idle CPU, multi-device route, support mailbox, Store approval, QUIC/TLS proof, and relay/tunnel transport remain open. |
...
| Item | Current value |
|---|---|
| Latest local release code commit before this documentation pass | `52698c4406de4747b4e1ce1834cfbad1cb0c75c1` after runtime relay fallback lease request wiring |
...
| Latest single-machine evidence | `docs\evidence\single-machine\1.15.0-rc.1\20260601-113438-HUGH_SECOND.evidence.json` |
| Single-machine output | `MUSU_RELEASE_SMOKE_OK_20260601_113412`; CLI route checked |
| Dashboard task | `7527eda1-b7f6-4a0d-8447-b17e76e756fe` |
...
| Runtime idle CPU evidence | primary clean desktop-open evidence passed at `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260601-113022-HUGH_SECOND.desktop-open.evidence.json` with `git_dirty=false`, one `musu-desktop`, six owned WebView2 helpers, owned Node `0`, max one-core CPU `musu=0`, `webview2=0.16`, and working set `343.16MB`; public readiness still requires the same clean evidence on the second PC |
...
12. **musu.pro network role**: the product now has hosted registry/rendezvous/route-evidence/relay-lease policy wiring and a runtime fail-closed relay lease request after direct failure. Direct LAN/manual peers remain valid, but cannot be the only public multi-device setup story. Relay payload transport remains unwired and must not become the default data path.
