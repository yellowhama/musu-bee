...
|---|---:|---|
| Single-machine Windows local beta | ~91% | Functional smoke, process ownership, repeated startup reuse, current primary packaged desktop/WebView2 idle CPU evidence, and current primary 4-state CPU matrix evidence are backed locally; desktop shell accumulation under repeated manual launches still needs window-reactivation/single-instance hardening; second-PC desktop-open CPU, second-PC matrix, and real route evidence are still missing. |
...
| Public desktop release readiness | ~69% | MSIX desktop entrypoint, source-fresh primary CPU proof, primary 4-state CPU matrix, public site branding/scroll regression coverage, runtime relay lease request wiring, persisted relay fallback evidence, P2P auth hash-allowlist code, and frontend polling hardening are stronger, but clean two-machine desktop/WebView2 idle CPU, second-PC matrix, multi-device route, support mailbox, Store approval, QUIC/TLS proof, relay/tunnel transport, production P2P env/live verification, and the latest site deploy recheck remain open. |
...
| Item | Current value |
|---|---|
| Latest local release code commit before this documentation pass | `f89a67f9d45c98d6bd131fea330d6a4bc7633aa2` after site follow-up and current evidence commits |
| Latest smoke source commit | `9f9e35ac0eefe1eb647cc62c13f6fb156b1ed62d` |
| Working tree | clean go/no-go on commit `1ab5a824be1291ac56667bc331474a7d63864d29` reports `manifest_dirty=false`; later documentation-only updates must preserve this state |
| Latest single-machine evidence | `docs\evidence\single-machine\1.15.0-rc.1\20260601-203715-HUGH_SECOND.evidence.json` |
| Single-machine output | `MUSU_RELEASE_SMOKE_OK_20260601_203657`; CLI route checked |
| Dashboard task | `4345c783-ddc2-4f7c-9328-6a721e7b3f1c` |
...
| Runtime idle CPU evidence | primary clean desktop-open evidence passed at `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260601-203537-HUGH_SECOND.desktop-open.evidence.json` with `git_dirty=false`, repo Node `1`, max one-core CPU `musu=0`, `node=0.03`, `webview2=0.05`, working set `655.4MB`, and no hot processes; stale packaged desktop shells were present in this first sample, so window reactivation/single-instance remains a hardening task; public readiness still requires the same clean evidence on the second PC |
| Runtime CPU scenario matrix evidence | primary clean 4-state matrix passed at `docs\evidence\runtime-cpu-scenarios\1.15.0-rc.1\20260601-203835-HUGH_SECOND.runtime-cpu-scenario-matrix.json` with route token `MUSU_CPU_SCENARIO_ROUTE_OK_20260601_203835`; second-PC matrix evidence is still required |
...
| Latest go/no-go after source commit | `ready=false`, `single_machine=true`, runtime idle CPU `1/2`, runtime CPU scenario matrix `1/2`, multi-device false, support false, Store false, `manifest_dirty=false` on commit `1ab5a824be1291ac56667bc331474a7d63864d29` |
| Public site deployment | Existing live `musu.pro` deploy passed earlier for scroll/logo/accent; the new common-shell accent marker/test expansion is local only until pushed, deployed by Vercel, and live-rechecked |
