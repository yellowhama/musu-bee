...
| Relay lease production audit | `musu relay status --json` shows logged-in control-plane wiring; `musu relay leases --json` against current live `https://musu.pro` still fails with `p2p_control_auth_not_configured`, but code now supports `MUSU_P2P_CONTROL_TOKEN_SHA256S`; production env configuration and live recheck are still required |
