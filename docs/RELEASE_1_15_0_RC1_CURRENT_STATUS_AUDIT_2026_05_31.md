...
2026-06-02 00:55 KST desktop single-instance gate addendum: packaged Start-menu/AppsFolder repeated activation is now an explicit go/no-go gate. `scripts\windows\audit-musu-desktop-single-instance.ps1` writes `musu.desktop_single_instance_audit.v1`, resolves the installed AppUserModelId, launches the packaged app through `shell:AppsFolder\<AppUserModelId>`, and records `musu-desktop.exe` before/after process counts. Current installed package `Yellowhama.MUSU_1.15.0.0_x64__ygcjq669as2b6` still fails even though source has the Tauri single-instance plugin: evidence `.local-build\desktop-single-instance\musu-desktop-single-instance-20260602-005439-HUGH_SECOND.json` recorded repeat count `3`, before `1`, after `4`, new shells `3`, `ok=false`, `fail_count=2`, then cleaned extra shells. `write-release-go-no-go.ps1` now reports `desktop_single_instance_verified=false` and blockers include `desktop-single-instance`; final handoff status and final operator packet verification include the new gate. The next release packaging step is a fresh MSIX build/install from current source, then re-run desktop single-instance, process ownership, single-machine smoke, desktop-open CPU, and CPU matrix evidence.

2026-06-02 08:40 KST addendum: after frontend polling timeout hardening, a fresh `local-sideload-manual` MSIX was built, installed, and verified on `HUGH_SECOND`. Current packaged desktop repeated activation passes at `docs\evidence\desktop-single-instance\1.15.0-rc.1\20260602-0832-HUGH_SECOND.desktop-single-instance.json` with repeat count `3`, before `1`, after `1`, and new desktop shells `0`. Process ownership passes at `docs\evidence\process-ownership\1.15.0-rc.1\20260602-0832-HUGH_SECOND.process-ownership.json`: one MUSU runtime, one desktop shell, zero MUSU-owned Node helpers, six owned WebView2 helpers, bridge PID `14556`, and `/health` HTTP 200 at `127.0.0.1:9967`. Single-machine smoke passes at `docs\evidence\single-machine\1.15.0-rc.1\20260602-083131-HUGH_SECOND.evidence.json` with dashboard task `4ae56776-f54d-4955-98cb-d6774626d072`, bridge `http://127.0.0.1:9967`, dashboard output `MUSU_RELEASE_SMOKE_OK_20260602_083131`, and CLI route checked. Clean `desktop-open` idle CPU evidence passes at `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260602-0833-HUGH_SECOND.desktop-open.evidence.json` from commit `22ba6c313dea4dd32ae43a46dca424b3443edf85`: 60.063s, `git_dirty=false`, MUSU `2`, repo Node `1`, owned WebView2 `6`, max one-core CPU `musu=0`, `node=0.08`, `webview2=0.34`, working set `498.42MB`, private memory `329MB`, and no hot processes. The clean 4-state matrix passes at `docs\evidence\runtime-cpu-scenarios\1.15.0-rc.1\20260602-083314-HUGH_SECOND.runtime-cpu-scenario-matrix.json`; all four required scenarios passed 60s samples with route token `MUSU_CPU_SCENARIO_ROUTE_OK_20260602_083314`. The operator-reported busy-loop is still not reproduced in MUSU-owned primary-machine evidence. Remaining release blockers are second-PC runtime/route evidence, live `musu.pro` P2P KV/control-plane evidence, `musu@musu.pro` delivery evidence, and Store/Partner Center evidence. Local operator caveat: this dev shell still has `C:\Users\empty\.cargo\bin\musu.exe` before the WindowsApps alias, so final packaged CLI checks should use the explicit WindowsApps alias or clean the stale dev alias.

2026-06-02 09:47 KST addendum: local API auth docs were corrected to match the current Rust bridge contract. `musu-rs/src/bridge/config.rs` already defaults `localhost_auth_required=true`, and `musu-rs/src/bridge/auth.rs` only bypasses loopback auth when `MUSU_BRIDGE_LOCALHOST_AUTH=0`/`false`/`no` explicitly opts into local development bypass. Current API/config/getting-started/manual/production/troubleshooting/architecture docs no longer claim localhost bypasses auth by default or instruct operators to set `MUSU_BRIDGE_LOCALHOST_AUTH=1` to require auth. New audit tool `scripts\windows\audit-local-api-auth-contract.ps1` emits `musu.local_api_auth_contract.v1` and passed with `ok=true`, `fail_count=0`, and `stale_doc_hit_count=0`; final operator packets now include and self-check the audit. Public release status is unchanged: this fixes a security documentation/handoff risk, not the remaining second-PC/P2P/support/Store evidence blockers.
...
| Item | Current value |
|---|---|
| Latest local release code commit before this documentation pass | `22ba6c313dea4dd32ae43a46dca424b3443edf85` after frontend polling timeout hardening and fresh packaged primary evidence refresh; later documentation/evidence-only commits are acceptable freshness deltas |
...
| Latest single-machine evidence | `docs\evidence\single-machine\1.15.0-rc.1\20260602-083131-HUGH_SECOND.evidence.json` |
| Single-machine output | `MUSU_RELEASE_SMOKE_OK_20260602_083131`; CLI route checked |
| Dashboard task | `4ae56776-f54d-4955-98cb-d6774626d072` |
| Bridge URL | `http://127.0.0.1:9967` |
...
| Runtime idle CPU evidence | primary clean desktop-open evidence passed at `docs\evidence\runtime-idle-cpu\1.15.0-rc.1\20260602-0833-HUGH_SECOND.desktop-open.evidence.json` with `git_dirty=false`, MUSU `2`, repo Node `1`, owned WebView2 `6`, max one-core CPU `musu=0`, `node=0.08`, `webview2=0.34`, working set `498.42MB`, private memory `329MB`, and no hot processes; public readiness still requires the same clean evidence on the second PC |
| Runtime CPU scenario matrix evidence | primary clean 4-state matrix passed at `docs\evidence\runtime-cpu-scenarios\1.15.0-rc.1\20260602-083314-HUGH_SECOND.runtime-cpu-scenario-matrix.json` with route token `MUSU_CPU_SCENARIO_ROUTE_OK_20260602_083314`, no hot processes, dashboard URL launch proof, and all four scenarios under budget; second-PC matrix evidence is still required |
...
| Local API auth contract | `scripts\windows\audit-local-api-auth-contract.ps1 -FailOnProblem -Json` passed with schema `musu.local_api_auth_contract.v1`, `ok=true`, `fail_count=0`, and `stale_doc_hit_count=0`; localhost bridge API auth is required by default and `MUSU_BRIDGE_LOCALHOST_AUTH=0` is only explicit trusted local dev bypass |
| Latest go/no-go after evidence/docs commit | commit `300b5b4419d728ceac58c38fc1597b02acb5d3c3` reports `ready=false`, `manifest_dirty=false`, `single_machine=true`, runtime idle CPU `1/2 [HUGH_SECOND]`, runtime CPU scenario matrix `1/2 [HUGH_SECOND]`, public metadata ok, MSIX install ok, MSIX desktop entrypoint ok, process ownership ok, startup single-instance ok, desktop single-instance ok, multi-device false, support false, Store false, and `p2p_control_plane_verified=false`. Remaining blockers are second-PC multi-device evidence, second-PC runtime idle CPU, second-PC CPU matrix, `musu@musu.pro` delivery evidence, Store/Partner Center evidence, and live `musu.pro` P2P owner-scoped relay lease evidence. |
...
| Desktop single-instance evidence | current installed package fails repeated activation at `.local-build\desktop-single-instance\musu-desktop-single-instance-20260602-005439-HUGH_SECOND.json`: repeat count `3`, before `1`, after `4`, new shells `3`, `ok=false`; source has `tauri-plugin-single-instance`, but fresh installed MSIX evidence is still required |
...
9. **Local API auth contract**: localhost bridge API requests require bearer auth by default. `MUSU_BRIDGE_LOCALHOST_AUTH=0` is only an explicit trusted local development bypass and must not be documented as production guidance.
...
14. **Startup and desktop single-instance policy**: repeated `musu up`, desktop Start Runtime, Store StartupTask/manual-launch overlap, and packaged Start-menu/AppsFolder activation must not spawn duplicate runtime or desktop owners. The release now has separate gates for CLI startup reuse (`musu.startup_single_instance_audit.v1`) and packaged desktop repeated activation (`musu.desktop_single_instance_audit.v1`).
