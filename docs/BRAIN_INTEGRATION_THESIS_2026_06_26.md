...
### 열린 스펙 충돌: brain data root

이 thesis와 현재 package proof는 `~/.musu/brain`을 사용한다. 반면 최신
brain handoff는 brain 쪽 기본 레이아웃을 `~/.musubrain` 계열로 설명한다.
둘 다 맞다고 두면 recall store가 갈라진다.

다음 brain 통합 작업의 첫 번째 선결조건:

1. MUSU가 단일 resolver/env contract를 소유한다.
2. Tauri sidecar, runtime bridge env, proof scripts, docs가 같은 root를 본다.
3. MSIX LocalState 밖이라는 안전 규칙은 유지한다.
4. root 변경이 필요하면 migration/compat proof를 먼저 만든다.

이 충돌이 해결되기 전에는 cockpit recall/capture UX와 자동수집 확대를
release-complete로 주장하지 않는다.
