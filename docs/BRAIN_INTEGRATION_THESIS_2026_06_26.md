...
2. token file ACL hardening verifier를 추가한다. 현재 token은 `~/.musu/brain/runtime/musu-ingest.token`에 저장되며
   값은 로그에 남기지 않지만, Windows owner-only ACL 검증은 아직 별도 게이트로 남아 있다.
