...
남은 핵심은 같은 `node_name` 후보의 bounded race/probe, TTL prune/reconcile이다.
...
- **musu는 이미 절반 이상 완성**: node_name 안정 식별자+레지스트리 진실원천(V30), mDNS 발견(V27
  ...
  모든 usable LAN interface 후보 publish, cache resolver 후보 확장. ✅ 코드 확인.
...
2. 연결 측에서 같은 `node_name` 후보들을 순차 선택이 아니라 bounded race/probe로 채택하게 한다.
3. 서버측 heartbeat TTL prune + boot reconcile로 stale registry/cache/manual peer를 정리한다.
