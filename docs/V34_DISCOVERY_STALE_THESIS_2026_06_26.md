...
`online_nodes=1`, `direct_healthy_nodes=1`, ACL checks pass로 확인됐다. 2026-06-27 production
follow-up 이후 같은 verifier는 `warn_count=0`, `remote_cloud_warning_count=0`로 통과한다.
guarded publisher가 `desktop-latest` rc.21 assets를 업로드했고, Vercel production deployment
`dpl_E7TrT4SfZm2kEaVvnM4DpW43i9nj`가 `https://musu.pro`에 alias됐다. live `/api/health`,
`/api/public-config`, `/install.ps1`, `verify-musu-pro-install-channel.ps1 -Json` 모두
`1.15.0-rc.21` 기준 통과한다. 즉 다른 PC 설치용 one-line installer 경로는 열린 상태다.
...
### 2026-06-27 rc.22 physical two-PC proof status
당시 관측된 `hugh-main` stale/loopback 상태는 rc.22 public install/proof path로 복구 검증됐다.
Hosted `fleet-proof.ps1`를 `hugh-main`에서 실행한 결과
`schema=musu.fleet_node_proof.v1`, `ok=true`, `fail_count=0`, `warn_count=0`,
`installed_package_version=1.15.0.22`, `bridge_bind_addr=0.0.0.0:4387`,
`advertised_public_url=http://192.168.1.192:4387`, `online_nodes=2`,
`direct_healthy_nodes=2`, expected peer `hugh_second`, and
`brain_ingest_token_acl_restricted` passing. Evidence:
...
observed-IP additive, TTL prune, mDNS 1순위)는 여전히 근본 설계 과제다. 현재 rc.22 direct fleet
증거는 green이지만, 이는 V34의 더 넓은 stale-recovery/route-race 설계 과제를 대체하지 않는다.
...
## 다음 행동
1. rc.22 기준 `hugh-main` 최신 WindowsApps/MSIX runtime 재시작/재설치와 LAN URL 재publish는
   hosted `fleet-proof.ps1` evidence로 완료됐다. 이후 package/node/route 변경 시 같은 proof를 재실행한다.
2. forward-time route preflight를 두 물리 머신에서 검증한다: stale 첫 후보가 있어도 reachable LAN 후보가
   먼저 선택되고, task POST가 중복 실행되지 않아야 한다.
3. production deploy 후 서버측 heartbeat presence TTL이 `hugh-main` stale row를 숨기는지 확인한다.
   2026-06-28 현재 boot/local reconcile 소스 경로는 구현됨:
   `musu-rs/src/peer/discovery.rs`의
   `reconcile_manual_peers_with_cached_registry`가 valid cache를 서버 truth로
   사용해 same-name stale manual ghost를 bridge startup 전에 정리하고, candidate-set
   중 현재 주소는 보존한다. 남은 것은 physical two-node proof와
   `verify-v34-self-heal-proof.ps1` 통과 evidence다.
