...
남은 핵심은 모든 LAN IP 후보 수집, 같은 `node_name` 후보의 bounded race/probe, TTL prune/reconcile이다.
...
2. local bridge가 모든 usable LAN IP 후보를 수집해 `meta.candidate_endpoints`에 넣도록 구현한다.
3. 연결 측에서 같은 `node_name` 후보들을 순차 선택이 아니라 bounded race/probe로 채택하게 한다.
4. 서버측 heartbeat TTL prune + boot reconcile로 stale registry/cache/manual peer를 정리한다.
5. (사용자/다음 세션) 이 thesis로 **V34 마스터 플랜 작성** — plan mode, agent-team, Phase 0 Researcher 2축.
6. 임시 복구(원하면): hugh-main 재시작 → LAN IP 재등록 시도(땜질). 근본은 V34.
