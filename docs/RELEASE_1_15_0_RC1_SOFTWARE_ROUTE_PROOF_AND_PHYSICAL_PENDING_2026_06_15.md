...
| `tailscale_ping_verified` | The two nodes reach each other over the tailnet (100.64.0.0/10) | ⏳ requires a real `tailscale up --login-server` join. Not run here to preserve this host's existing Tailscale.com connection. |
| `physical_host_distinct` | Source and target are **different physical hosts** (distinct OS hostnames) | ❌ inherently needs a second machine. |
