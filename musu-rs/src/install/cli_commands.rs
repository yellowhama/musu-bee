...
/// opens a browser). It is now split into [`initiate_device_flow`] +
/// [`poll_and_finalize`] so the Desktop `open` path (musu-startup) can drive the
/// same flow and call [`open_url`] for the browser deep-link.
...
/// the existing [`open_url`]; failure to open is non-fatal (the marker still
/// carries the URL/code).
///
